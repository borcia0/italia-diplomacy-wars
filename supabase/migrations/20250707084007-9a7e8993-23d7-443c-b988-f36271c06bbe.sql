...
-- Add missing columns for puzzle and target games to minigame_stats table
ALTER TABLE public.minigame_stats 
ADD COLUMN IF NOT EXISTS puzzle_plays_today integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS target_plays_today integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS last_puzzle_play timestamp with time zone,
ADD COLUMN IF NOT EXISTS last_target_play timestamp with time zone;

-- Update the existing functions to handle the new game types
CREATE OR REPLACE FUNCTION public.can_play_minigame(p_user_id uuid, p_game_type text)
RETURNS boolean AS $$
DECLARE
  v_stats RECORD;
...
BEGIN
  ...
  CASE p_game_type
    ...
    WHEN 'puzzle' THEN
      v_last_play := v_stats.last_puzzle_play;
      v_plays_today := COALESCE(v_stats.puzzle_plays_today, 0);
    WHEN 'target' THEN
      v_last_play := v_stats.last_target_play;
      v_plays_today := COALESCE(v_stats.target_plays_today, 0);
    ELSE
      RETURN FALSE;
  END CASE;
...
-- Update the stats function to handle new game types
CREATE OR REPLACE FUNCTION public.update_minigame_stats(p_user_id uuid, p_game_type text)
RETURNS void AS $$
BEGIN
  ...
  VALUES (
    ...
    CASE WHEN p_game_type = 'puzzle' THEN now() ELSE NULL END,
    CASE WHEN p_game_type = 'target' THEN now() ELSE NULL END,
    ...
    CASE WHEN p_game_type = 'puzzle' THEN 1 ELSE 0 END,
    CASE WHEN p_game_type = 'target' THEN 1 ELSE 0 END,
    now()
  )
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    last_puzzle_play = CASE WHEN p_game_type = 'puzzle' THEN now() ELSE minigame_stats.last_puzzle_play END,
    last_target_play = CASE WHEN p_game_type = 'target' THEN now() ELSE minigame_stats.last_target_play END,
    ...
    puzzle_plays_today = CASE WHEN p_game_type = 'puzzle' THEN COALESCE(minigame_stats.puzzle_plays_today, 0) + 1 ELSE minigame_stats.puzzle_plays_today END,
    target_plays_today = CASE WHEN p_game_type = 'target' THEN COALESCE(minigame_stats.target_plays_today, 0) + 1 ELSE minigame_stats.target_plays_today END,
    updated_at = now();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Update the reset function to handle new game types
